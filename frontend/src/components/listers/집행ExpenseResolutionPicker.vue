                ...
                <v-list-item v-for="(item, idx) in list" :key="idx">
                    <template v-slot:default="{ active }">
                        ...
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle>
                                DirectOrIndirectExpense :  {{item.directOrIndirectExpense }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                Item :  {{item.item }}
                            </v-list-item-subtitle>
                            ...
                            <v-list-item-subtitle>
                                ResolutionContent :  {{item.resolutionContent }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
...
<script>
    ...
    export default {
        name: '집행ExpenseResolutionPicker',
        props: {
        ...
        async created() {
            ...
            if(temp.data) {
                me.list = temp.data._embedded.expenseResolutions;
            }

            if(me.value && typeof me.value == "object" && Object.values(me.value)[0]) {
                var id = Object.values(me.value)[0];
                var tmpValue = await axios.get(axios.fixUrl('/expenseResolutions/' + id))
                if(tmpValue.data) {
